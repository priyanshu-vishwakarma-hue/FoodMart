...
function TopRestaurant({ content,title}) {
  ...
  return (
    <div className="w-full overflow-hidden flex flex-col  mt-10">
      <div className="flex justify-between">
        <h1 className=" text-[21px] font-extrabold mb-3">
          {title}
        </h1>
        <div className=" flex gap-5 items-center pb-3">
          <div
            onClick={() => setmove((prev) => (prev >= 0 ? prev : prev + 38.5))}
            className={
          ...
          <div
            onClick={() =>
              setmove((prev) => (prev <= -304 ? prev : prev - 38.5))
            }
            className={
              "rounded-3xl  w-9 h-9 flex justify-center items-center cursor-pointer " +
              (move <= -304 ? "bg-gray-200 text-gray-400" : "bg-gray-300")
            }
          >
      ...
      <div className="w-full overflow-hidden">
        <div
          ...
          className=" flex gap-8 duration-400 "
        >
          {content?.map(({ info, cta: { link } }) => {
            ...
            return (
              <Link to={`restaurant/${link}`}>
                <div className="w-[280px] h-[300px] hover:scale-95 duration-250" onClick={()=>HandleRestrant(info,element,link)}>
